...
class BackPropagation{
	public static final double LEANING_RATE = 0.5;
	...
	public static final double SigmoidFunctionDifferential(double output) {
		return (1-output)*output;
	}
	public static final double SigmoidFunction(double input) {
	...
	public void Calculating(int testCase) {
		...
		while(!queueNode.isEmpty()){
			Node node = queueNode.poll();
			node.setResult(SigmoidFunction(node.getResult()));
			for(int j = 0; j<node.getLinkedFrontNodes().size(); j++) {
	...
	public void OutputNodeDelta(Node node, int idx) {
		node.setDelta(error[idx]*SigmoidFunctionDifferential(node.getResult()));
	}
	public void HiddenNodeDelta(Node node) {
		...
		hiddenNodeWDifferential *= SigmoidFunctionDifferential(node.getResult());
